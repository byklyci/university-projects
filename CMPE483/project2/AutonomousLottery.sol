...
contract AutonomousLottery {
    ...
    event BuyEvent(uint ticket, uint weekNumber);
    event RevealEvent(uint ticket, bool revealed, bool refunded);
    event WithdrawEvent(uint ticket, uint prize);
        ...
        returns(uint weekNumber)
    {
        ...
        emit BuyEvent(ticket, lotteries.length);
    ...
    function reveal(uint ticket, uint randomNum) public returns(bool revealed, bool refunded) {
        ...
        if(lotteries[lotteries.length-1].collectedMoney < SUM_OF_PRIZES) {
            ...
            if(lotteries[lotteries.length-1].tickets[ticket].refunded){
                emit RevealEvent(ticket, false, false);
                return (false,false);
            }else{
                lotteries[lotteries.length-1].tickets[ticket].refunded = true;
                msg.sender.transfer(2 ether);
                emit RevealEvent(ticket, false, true);
                return (false,true);
            }
            ...
            if(
                ...
                emit RevealEvent(ticket, true, false);
                return (true,false);
            } else {
        ...
        prizeNotCollected(lotteryWeek,ticketNumber)
    {
        ...
        emit WithdrawEvent(ticketNumber,prize);
    }
